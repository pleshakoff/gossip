...
Для репликации данных используется алгоритм GOSSIP.
...
Каждая серверная имеет доступ к БД в которой хранятся непосредственно данные. 
...
в качестве инедекса БД используется HashSet (для contains  O(1))    
...
В текущей реализация выбрана стратегия pull.  
...
в в специальном векторе, любое изменение данных ноды приводит к увеличению версии векторе.
...
Так версия версия ноды поставщика в векторе поставщика не больше версии ноды получателя+1    
* 1 [1,0]
* 2 [1,2]  
...
А в docker-compose.yml в параметрвх запуска для каждый ноды указывается свой профиль.
